C     ******************************************************************
C     FARM PROCESS VERSION 3.0 (FMP3) FOR MODFLOW-2005 VERSION 1.6_01
C     11/30/2011
C     ******************************************************************
C
...
C-------ONE-DIMENSIONAL VARIABLES
     ...
     2  FALLOT,QMAXFW,QFRACF                                                     ! Added QMAXFW to hold original QMAXF values and QFRACF to hold ratio QMAXFW/ALLTOGW --rth
        LOGICAL,SAVE,DIMENSION(:),POINTER:: LFID                                !Added Logical Array to keep track of what Farms are active in current stress period --rth
C
C-------TWO-DIMENSIONAL VARIABLES
        INTEGER,SAVE,DIMENSION(:,:),POINTER::
     1  IFID,IFIDOLD,ICID,ISID,IFALLOW,IFDRID,IFCRID,ISRD,ISRR  
        DOUBLE PRECISION,SAVE,DIMENSION(:,:),POINTER::                  !FORMERLY REAL
...
C-------ONE-DIMENSIONAL VARIABLES
     ...
     1  QMAXF,QREQ,QREQOLD,TFDR,TFDROLD,FLOWINMIN,QSTRMIN,QXTF,QSAVE,           !Added QXTF to keep track of total reduced pumpage for each farm well if NWT-smoothing ena ddeficit irrigation is used rth
     2  FALLOT,QMAXFW,QFRACF                                               ! Added QMAXFW to hold original QMAXF values and QFRACF to hold ratio QMAXFW/ALLTOGW--rth
        LOGICAL,DIMENSION(:),POINTER:: LFID                                !Added Logical Array to keep track of what Farms are active in current stress period --rth
C
C-------TWO-DIMENSIONAL VARIABLES
...
C
C
      SUBROUTINE FMP2AR(IN,IUNITSFR,IUNITMNW,IUNITUZF,IUNITNWT,IGRID,
     &                  ILGR)
...
C        ARGUMENTS:
C     ------------------------------------------------------------------
      INTEGER IN,IUNITSFR,IUNITMNW,IUNITUZF,IGRID,ILGR,IUNITNWT
      INTEGER NPP,MXVL                                                  !Added for NWT connection to Farm wells by rth
...
C ------LGR EXCLUSIONS
      IF(ILGR.GT.0) THEN
        ...
        ELSEIF(IALLOTSW.GT.1) THEN
          WRITE(IOUT,24) "PRIOR APPROPRATION (RESET IALLOTSW=0 OR 1)"
          STOP
        ENDIF
...
c       (QMXF,QREQ,QAVF,QDEF,QEXC)  and initialize
     ...
     &QMAXFW(NFARMS),QFRACF(NFARMS))                                                  ! added QMAXFW to hold original QMAF values and QFRACF to holde fraction QMAXFW/ALLOTGW
      QMAXF=0D0
      QREQ=0D0
      QREQOLD=0D0
      QMAXFW=0D0
...
C
C
      SUBROUTINE FMP2RQ(IN,IUNITSFR,IUNITMNW,IUNITNWT,IGRID,ILGR)
...
C     ------------------------------------------------------------------
     ...
     6    ,IFRMFL,LFID,IALLOTGW,ALLOTGW                                                             !Multiple WBS --rth
...
C        ARGUMENTS:
C     ------------------------------------------------------------------
      INTEGER IGRID,IN,IUNITMNW,IUNITSFR,ILGR,IUNITNWT                         !added IUNITNWT for NWT link to Farm Wells
C     ------------------------------------------------------------------
C        LOCAL VARIABLES:
c     ------------------------------------------------------------------
      CHARACTER*200 LINE
...
c     1      (NPFWL.GT.1.AND.K.EQ.NPFWL) ) THEN
             ...
             IF(INDEX(LINE,' P ').GT.0) THEN
               ...
               WRITE(9999,'(A)') LINE
               REWIND(9999)
               CALL UPARLSTRP(LSTSUM,MXFWEL,9999,IOUT,IP,'FWEL','QMAX',
     1              1,NUMINST)
...
c              IF(LINE(ISTART:ISTOP).EQ.'P') THEN
               IF(NLST.EQ.0) THEN
                WRITE(IOUT,*)
                CALL ULSTLB(IOUT,
     1'WELL NO. LAYER    ROW    COL  F-WELL ID    FARM ID      QMAXfac',
     2          FWLAUX,5,NAUX)
                GOTO 4
              ELSE
C                BACKSPACE IN !REWIND TO BEGINNING OF LINE
     ...
     3        FWLAUX,5,NAUX,NCOL,NROW,NLAY,6,6,IPFWEL,IUNITMNW,IUNITNWT)
              ENDIF
            ELSE
            CALL FMP2WELRD(NLST,FWELL,LSTBEG,NFWLVL,MXFWEL,IFWLAL+1,IN,
     1         IOUT,
    ...
    4       IF(ILGR.NE.0.AND.IGRID.GT.1.AND.
     ...
     2         (NPFWL.GT.1.AND.K.EQ.NPFWL.AND.NLST.EQ.0)) ) THEN
               ...
               CALL FMP2PWELTOCWEL(2,IL,LSTBEG)
...
C2A-----PULL CHILD GSE FROM PARENT MODEL        
          CALL FMP2PRTOCH_BILINEAR(FMPDAT(1)%GSURF,GSURF)
        ELSE
...
CC3A2---PULL CHILD FID FROM PARENT MODEL        
        ...
        ELSE
          ...
          do ii=1,nrow
           do jj=1,ncol
            if(IFID(jj,ii).gt.0)LFID(IFID(jj,ii))=.true.
           enddo
          enddo
        ENDIF
...
C             Net-Recharge will not be calculated for such cells!
      IF(IROTFL.GE.0) THEN                                
      IF(ILGR.NE.0.AND.ILGR.NE.0.AND.IGRID.GT.1) THEN            ! What does this mean --rth
        READ(IN,'(A)') LINE
        LLOC=1
        CALL URWORD(LINE,LLOC,ISTART,ISTOP,1,N,R,IOUT,IN)
        IF(LINE(ISTART:ISTOP).EQ.'P') THEN
...
C       ROOT ZONE DEPTH IS TO BE READ AS A CONSTANT FOR THE ENTIRE SIMULATION
      IF(IRTFL.EQ.1) CALL FMP2LSTRD(ROOT,2,NCROPS,IN,IOUT,2,2)
...
C       (ONE LUMPED VALUE PER ENTIRE SIMULATION)
      IF(IFTEFL.EQ.1) CALL FMP2DPLSTRD(FTE,4,NCROPS,IN,IOUT,2,4)
...
C5D1----READ FRACTIONS OF IN-EFFICIENT LOSSES TO SW-RUNOFF FROM PRECIPITATION AND FROM IRRIGATION
      IF(IIESWFL.EQ.1) CALL FMP2DPLSTRD(FIESW,3,NCROPS,IN,IOUT,2,3)
...
ccrth
      IF(IALLOTGW.EQ.1) THEN
        CALL FMP2DPLSTRD(ALLOTGW,2,NFARMS,IN,IOUT,2,2)       
      ENDIF
...
C
C     
      SUBROUTINE FMP2RP(IN,KPER,IUNITSFR,IUNITMNW,IUNITNWT,IGRID,NGRIDS,
     1  ILGR,IOUF)
...
C     -------------------------------------------------------------------
     ...
     8  ALLOTGW,QMAXFW,QFRACF                                                   Added GW allotment arrays -- rth
...
C        ARGUMENTS:
C     -------------------------------------------------------------------
      INTEGER IGRID,IUNITSFR,IN,IUNITMNW,IUNITNWT,KPER,ILGR,NGRIDS,IOUF
C     -------------------------------------------------------------------
C        LOCAL VARIABLES:
      CHARACTER*200 LINE
      CHARACTER*24 ANAME(4)
...
c       (info: if consumptive use changes at each time-step, then istartfl is set zero in FMP2AD.)
      ISTARTFL=0
      if(IFRMFL.eq.2)LFID=.false.                                         !For changing Farms always reset pointers back to false for each stress period --rth
c
...
C         IF(ITMP.EQ.-2) THEN
     ...
     1'WELL NO. LAYER    ROW    COL  F-WELL ID    FARM ID         QMAX',
     2        FWLAUX,5,NAUX,NCOL,NROW,NLAY,6,6,IPFWEL,IUNITMNW,IUNITNWT)
         ENDIF
C         IF(ILGR.NE.0.AND.IGRID.GT.1) THEN
...
c                 CALL FMP2PWELTOCWEL(2,L)
c              ENDIF
c           ENDIF
c         ENDDO
c         ENDDO
c         ENDIF
C         IF(IGRID.GT.1) CALL FMP2PWELTOCWEL(2,0) 
   30    CONTINUE
      END IF
C
...
C
         DO L=1,NFWELS
         IF(IUNITMNW.GT.0.AND.IDINT(FWELL(4,L)).LT.0.AND.IALLOTSW.GT.1)
     1   THEN
...
ccrth
      IF(IALLOTGW.GT.0)THEN
       DO NF=1,NFARMS
        IF(LFID(NF))THEN
            QMAXF(NF)=ALLOTGW(2,NF)                                           !RESET QMAXF for each Farm equal to the Groundwater Allotment either for entire simulation or at each stress period
      ...
      WRITE(IOUT,103) 
  103 FORMAT(1X,/1X,
     1'FARM-ID  NUMBER OF WELLS PER FARM  GROUNDWATER-ALLOTMENT ',
     2'           FRACTION OF PUMPING CAPACITY PER FARM')
      Do NF=1,NFARMS
      WRITE(IOUT,104) NF, NWPERF(NF), ALLOTGW(2,NF), QFRACF(NF)
  104 FORMAT(4X,I4,20X,I6,9X,2F20.4)
      ENDDO
...
C       (IEBFL=2: cell-by-cell efficiency may vary over time step)
     ...
     1  (IEBFL.EQ.0.OR.IEBFL.EQ.2).OR.IROTFL.EQ.-1.or.IFRMFL.eq.2)THEN                !NOT NECESSARY FOR (IEFFL=1 AND IEBFL=1) --> ALREADY IN RQ
      ...
      IF(IFA(NF).NE.0) THEN                                             !CALC. EFF. TO SPECIFIED EFFICIENCIES AT BEGINNING OF STRESS PERIOD!
...
C     FARM CANAL AND DRAIN REACH IDs IN THE PARENT CAN BE ASSIGNED FOR CHILD MODELS WITH AN SFR REPRESENTED SOLELY BY THE PARENT.
     ...
     1          IC.GE.LGRDAT(N)%NPCBEG.AND.IC.LE.LGRDAT(N)%NPCEND) THEN
                IF(LGRDAT(N)%ISFRGRID.EQ.N) THEN
                  IFID(IC,IR)=0
                  LFID(IFID(IC,IR))=.false.                                     !disable pointers for farm ID's the are coincident with child models --rth
                ELSE
...
C
     ...
     1  (FMPDAT(1)%ICUFL.EQ.1.OR.FMPDAT(1)%ICUFL.EQ.-1)) THEN
        CALL FMP2PRTOCH_BILINEAR(FMPDAT(1)%ETR,ETR)
      ENDIF
...
C4D-----READ TRANSPIRATORY AND EVAPORATIVE FRACTIONS OF CROP CONSUMPTIVE USE
      IF(IFTEFL.EQ.2) CALL FMP2DPLSTRD(FTE,4,NCROPS,IN,IOUT,2,4)
...
C4E-----READ FRACTIONS OF IN-EFFICIENT LOSSES TO SW-RUNOFF FROM PRECIPITATION AND FROM IRRIGATION
      IF(IIESWFL.EQ.2) CALL FMP2DPLSTRD(FIESW,3,NCROPS,IN,IOUT,2,3)
...
C       FOR EVAPORATIVE AND TRANSPIRATORY PART OF PRECIPITATION PER CELL
      ...
      IF(IPFL.EQ.-1.AND.(FMPDAT(1)%IPFL.EQ.2)) THEN
         CALL FMP2PRTOCH_BILINEAR(FMPDAT(1)%PFLX,PFLX)
      ENDIF
...
C8D3----DEFINE NUMBER OF SEGMENT FOR EACH FARM, ONCE AN FARM HEADGATE REACH WITHIN (AUTOMATIC) OR REMOTE FROM (SPECIFIED) A FARM IS FOUND
     ...
     2     IFCRID(NF,L-2).AND.IFCRID(NF,L).NE.IFCRIDOLD) THEN
      ...
      ENDIF
C
      if(LFID(NF))CALL RETURNFLOW(NF,IOUF)                                      !Don't assign a return flow location for inactive farms--rth
C
...
C
C
      SUBROUTINE FMP2ADSUB(KPER,IGRID,IOUF)
C-----VERSION 2.01 07/29/2010 FMP2ADSUB
C*********************************************************************
...
C        SPECIFICATIONS:
C     -----------------------------------------------------------------
      USE FMPMODULE,    ONLY:GSURF,NFARMS,IIESWFL,IRTPFL
      USE FMPBLK,       ONLY:ZER
      USE GLOBAL,       ONLY:IOUT,NROW,NCOL,NLAY,IBOUND  
      USE GWFSUBMODULE, ONLY:DVZ
      IMPLICIT NONE
C     -----------------------------------------------------------------
C        ARGUMENTS:
C     -----------------------------------------------------------------
      INTEGER KPER,IGRID,IOUF
C     -----------------------------------------------------------------
C        LOCAL VARIABLES:
C     -----------------------------------------------------------------
      INTEGER IR,IC,IL,NF
...
C3-----RECHECK FOR LOWEST ELEVATION WITHIN ANY FARM AND NEAREST REACH
      DO NF=1,NFARMS
        IF(IRTPFL.NE.0) THEN
          WRITE(IOUF,'(//,"RECHECK RETURNFLOW INFORMATION FOR FARM: ",
     1    I8,", PERIOD ",I4,/,A51)') NF, KPER,
     2    "---------------------------------------------------"
        ENDIF
        CALL RETURNFLOW(NF,IOUF)
      ENDDO
C
      ENDIF
C
...
C     -----------------------------------------------------------------
     ...
     3    NFARMS,EFF,EF2D,IFA,ICA,ISTARTFL
...
C
C
      SUBROUTINE FMP2FM(KITER,KPER,KSTP,IUNITSFR,IUNITMNW,
     1                  IUNITUZF,IUNITNWT,ILGR,IGRID,NGRIDS,LGRITER)                !added IUNITNWT  by rth
...
C        ARGUMENTS:
C     ------------------------------------------------------------------
      INTEGER KITER,KPER,KSTP,IUNITSFR,IUNITMNW,IUNITUZF,ILGR,IUNITNWT,
     1 IGRID,NGRIDS,LGRITER,IJ,ISS
C     ------------------------------------------------------------------
...
C        LOCAL VARIABLES:
     ...
     9  RTFUNC,FUNC,QACT,QMAXMNW2,QMAXMNW3,FIESWI,FIESWP,TG,DRDC,
...
C     (THIS COULD SIT IN SOME FORM IN FMP2RQ ... BUT REQUIRES FMP TO BE IN NAME FILE OF CHILD MODEL)
...
C             FOR NON-MNW WELLS USED TOGETHER WITH MNW-WELLS TO RESET QMAXF, MAKE SURE QMAX=0 FOR IBOUND=0               
          ...
          ENDDO
        QMAXF(NF)=QSUMF
        QMAXF(NF)=QSUMF*QFRACF(NF)                                       !!Reset the QMAX with fraction with respect the potential GW-Allotment or 1 --rth 
...
C6G1----CALCULATE TOTAL FARM DELIVERY REQUIREMENT FOR EACH FARM
      ...
      TFDR(NF)=0.0D0
        DO IR=1,NROW
        DO IC=1,NCOL
           IF(IFID(IC,IR).EQ.IFA(NF).AND.ICID(IC,IR).GT.0) THEN
...
C
      ...
      IF(ICA(NC).NE.0) THEN     
         DO IR=1,NROW
         DO IC=1,NCOL 
            IF(IFID(IC,IR).EQ.NF.AND.
     1         ICID(IC,IR).EQ.INT(CROPBEN(1,NC))) THEN
...
C       THE RHS ACCUMULATOR.
      ELSEIF(Iunitnwt.GT.0.and.IDINT(FWELL(4,L)).GT.0)THEN      !currently only allow NWT-smoothing for deficit irrigation or water stacking options
       FWELL(NFWLVL,L)=Q
...
ccrth        IF (LAYTYPUPW(il).GT.0.and.(Hd-bbot).lt.0.1) THEN
          ...
          QXTF(ID) = QXTF(ID) + (QSAVE(L)-Qp)                 !Accumulate the reduced pumpage for all single-aquifer farm wells in each farm if using NWT with smoothing rth
ccrsn Derivative for RHS
          ij = Icell(IC,IR,IL)
          A(IA(ij)) = SNGL(DBLE(A(IA(ij))) + dQp)
      ELSEIF(LAYTYPUPW(IL).LE.0.or.SatDif.GT.SATTHK)THEN
...
C     ------------------------------------------------------------------
      ...
      USE GWFNWTMODULE, ONLY:Tol             !rth added for head closure criteria of NWT
      IMPLICIT NONE
C     ------------------------------------------------------------------
C        ARGUMENTS:      
C     ------------------------------------------------------------------
      INTEGER IUNITMNW,IUNITPCG,IUNITSIP,IUNITDE4,IUNITGMG,IUNITNWT
C     ------------------------------------------------------------------
C        LOCAL VARIABLES:
C     ------------------------------------------------------------------
...
C
C      
      SUBROUTINE FMP2WELBD(KSTP,KPER,IUNITMNW,IUNITUZF,IUNITNWT,IGRID)     !Added IunitNWT for Farm wells simualted in combination with NWT by rth
...
C     ------------------------------------------------------------------
     ...
     5    FMPDAT,PSIRAMPF,IUNITRAMPF,QXTF,QSAVE,SATTHK                                 !Added Variables for reducing pumpage in Farm Wells for NWT by rth
      USE FMPBLK
      USE GWFMNW1MODULE, ONLY:IWL2CB,NWELL2,WELL2
      USE GWFUZFMODULE, ONLY:IUZFBND,IRUNBND,EXCESPP,SEEPOUT,REJ_INF
      USE GLOBAL,       ONLY:NCOL,NROW,NLAY,IOUT,IBOUND,HNEW,BUFF,
     1                       ITMUNI,BOTM,LBOTM                       !Added BOTM and LBOTM for NWT connection with Farm wells by rth
...
C        ARGUMENTS:
C     ------------------------------------------------------------------
      INTEGER KSTP,KPER,IUNITMNW,IUNITUZF,IGRID,ISS
C     ------------------------------------------------------------------
C        LOCAL VARIABLES:
C     ------------------------------------------------------------------
     ...
     2 EP,TP,EI,TI,FDS(9),VAL(24),QMAXTOL,NDOUT,SRDOUT,RDOUT,QFOUT,Q             !added local variable Q for NWT connection to Farm Wells by rth
      ...
      INTEGER Iunitnwt, iw1                                                      !added for NWT connection to Farm Wells by rth
C     ------------------------------------------------------------------
      CALL SFMP2PNT(IGRID)
C
...
C       (TO BUILD TIME SERIES FOR FARMS: PER,STP,AND TIME ARE INCLUDED FOR EACH FARM)
      ...
      IF(IGRID.EQ.1.AND.KPER.EQ.1.AND.KSTP.EQ.1) WRITE(1001,31) TIMEUNIT
   31 FORMAT
     1(2X,'PER',2X,'STP',A14,3X,'GRID'4X,'FID',9X,'OFE',14X,
...
C       (TO BUILD TIME SERIES FOR FARMS: PER,STP, AND TIME ARE INCLUDED FOR EACH FARM)
    ...
    1 FORMAT(1X,'SAVING "FARM DEMAND AND SUPPLY" ON UNIT',I5,
     1'FOR GRID ',I5,' AT TIME STEP',I3,', STRESS PERIOD',I4)
...
C4A1A   HEADER FOR COMPACT FARM BUDGET
      ...
      IF(IGRID.EQ.1.AND.KPER.EQ.1.AND.KSTP.EQ.1) WRITE(1008,41) TIMEUNIT
   41     FORMAT(
     1  '         PER','         STP',A14           ,'        GRID',
     *  '         FID',
...
C4A1B   HEADER FOR DETAILED FARM BUDGET
      ...
      IF(IGRID.EQ.1.AND.KPER.EQ.1.AND.KSTP.EQ.1) WRITE(1008,42) TIMEUNIT
   42     FORMAT(
     1  '         PER','         STP',A14           ,'        GRID',
     *  '         FID',
...
C5D1----PRINT TO ASCII FILE
     ...
     1  (DABS(FDS(I)).LT.1D-1.AND.DABS(FDS(I)).GT.FPS)) THEN
      ...
      fmt_string=
     1'(2I5,A14,2I7,F12.4,'//TRIM(ADJUSTL(fmt_number))//'(X,A),I10)'
      WRITE(1001,FMT=TRIM(fmt_string))
     1KPER,KSTP,TIME,IGRID,NF,EFF(NCROPS+2,NF),(FDSVAL(I),I=1,9),IDEFFL
C
...
C6G----PRINT FARM BUDGET TO ASCII FILE
     ...
     1  (DABS(VFB(I,NF)).LT.1D-1.AND.DABS(VFB(I,NF)).GT.FPS)) THEN
      ...
      fmt_string=
     1'(2I12,A14,2I12,X,'//TRIM(ADJUSTL(fmt_number))//'(A,X))'
      WRITE(1008,FMT=TRIM(fmt_string))
     1 KPER,KSTP,TIME,IGRID,NF,(CVAL(I),I=1,IVAL*2)
C
...
C       (TO BUILD TIME SERIES FOR FARM WELLS: PER,STP,AND TIME ARE INCLUDED FOR EACH FARM WELL)
      ...
      IF(IGRID.EQ.1.AND.KPER.EQ.1.AND.KSTP.EQ.1) WRITE(1002,33) TIMEUNIT
   33 FORMAT
...
C8F3----GET FLOW RATE AS CALCULATED IN FM MODULE.
      QQ=FWELL(NFWLVL,L)
ccrth
      IF(KPER.eq.1.and.iw1.eq.1.and.Iunitnwt.ne.0)
     & OPEN(UNIT=1011, FILE='RED_FMP_PMP.OUT', STATUS='UNKNOWN')
cc
      IF(IDINT(FWELL(4,L)).GT.0.AND.                 !Only use single-aquifer FMP wells and not ones connected to MNW1 =>Farm-well ID is negative
...
! write wells with reduced pumping
      ...
      IF(QSAVE(L).LT.0)THEN
         ...
         ENDIF
      END IF !rgn
       IF ((Qp.LT.0.9999999D0.and.Qp.gt.0.D0).AND. Iunitnwt.NE.0 ) THEN
        IF ( iw1.EQ.1 ) THEN
         WRITE(1011,*)
         WRITE(1011,300)KPER,KSTP
         WRITE(1011,400)
        END IF
        WRITE(1011,500)KPER,KSTP,IL,IR,IC,REAL(QSAVE(L)),REAL(QQ),
     1                REAL(Hd),REAL(bbot),IDINT(FWELL(4,L)),
     2         IDINT(FWELL(5,L))
        iw1 = iw1 + 1
      END IF
...
C       (MULTI-AQUIFER WELL PUMPAGE INDICATED BY ZERO LAYER NUMBER)
      IF(IFWLCB.EQ.1) THEN
         IF(IUNITMNW.GT.0.AND.IDINT(FWELL(1,L)).EQ.0) IL=0
        WRITE(1002,64) KPER,KSTP,TIME,IGRID,
     1   IDINT(FWELL(4,L)),IDINT(FWELL(5,L)),IL,IR,IC,QQ
...
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE FMPMODULE,    ONLY:FNRCH,NFARMS,IFID,IFNRCB,IFA
...
C4B2----WRITE LIST OF CUMULATIVE FARM NET RECHARGE FOR EACH FARM
        ...
        FNRECH=0.D0         
          ...
          ENDDO
          ENDDO
        WRITE(1003,64) KPER,KSTP,TIME,IFA(NF),FNRECH
...
C       (TO BUILD TIME SERIES FOR FARMS: PER,STP, AND TIME ARE INCLUDED FOR EACH FARM)
   ...
   65     FORMAT(/,1X,'SAVING ',
     ...
     2    ' ON UNIT',I5,' AT TIME STEP',I3,', STRESS PERIOD',I4)      
          IF(KPER.EQ.1.AND.KSTP.EQ.1) THEN
...
C6A2----WRITE LIST OF CUMULATIVE FARM NET RECHARGE FOR EACH FARM
        ...
        FNRECH=0.D0         
          ...
          ENDDO
          IF(IFNRCB.EQ.3) THEN
             WRITE(1005) KPER,KSTP,TIME,IFA(NF),FNRECH
          ELSEIF(IFNRCB.GT.3.AND.IBD.EQ.1) THEN
             WRITE(IFNRCB) IFA(NF), FNRECH
          ENDIF
        ENDDO
C
C
...
C
     ...
     2  IUNITMNW,IUNITNWT)                                                  !added IUNITNWT for connection of NWT to Farm Wells
...
C     ------------------------------------------------------------------
     ...
     1  NROW,NLAY,ISCLOC1,ISCLOC2,ITERP,IUNITMNW,IUNITNWT
...
C7B-----If output unit is positive; write a message to output unit.
      ELSE IF(IOUT.GT.0) THEN
         ...
         END IF
201      FORMAT(1X,/1X,'FILE UNIT',I5,' : ERROR CONVERTING "',A,
     1       '" TO ',A,' IN LINE:',/1X,A)
...
C-----VERSION 2 10/15/2009 PRIOR
     ...
     1                       NFSEG,IRDRFL,PCLOSE,IPAPFL,ISTARTFL
...
c      
C     ******************************************************************
      SUBROUTINE FMP2PWELTOCWEL(ID,MM,IBEG)
C     ******************************************************************
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
...
C                       EXCLUDE PARENT WELLS FROM BEING USED FOR A CHILD MODEL FARM, IF EVEN ONE WELL IS SPECIFIED FOR THAT CHILD MODEL FARM
                        DO N=IBEG,MM-1
                         IF(FMPDAT(1)%FWELL(5,L).EQ.FWELL(5,N)) THEN
                           GOTO 206
                         ENDIF
                        ENDDO
C                        
...
C                       APPEND INFO TO LIST IN LIST FILE
     ...
     1                  (FWELL(N,M),N=6,NFWLVL-1)
 205                    FORMAT(1X,4I7,2I11,6ES16.4)
                     M=M+1
                     ENDIF
                  ENDIF
C                ENDDO
...
C     ******************************************************************
      SUBROUTINE FMP2PRTOCH_BILINEAR(P,F)
C     ******************************************************************
C        SPECIFICATIONS:
C     ------------------------------------------------------------------
      USE FMPMODULE,   ONLY:FMPDAT
