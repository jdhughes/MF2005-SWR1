...
C14-----READ SEGMENT INFORMATION FOR FIRST STRESS PERIOD.
      IF ( NSFRPAR.EQ.0 ) THEN
        ...
        lb = 1
        ichk = 1
        CALL SGWF2SFR7RDSEG(nlst, lb, In, Iunitgwt, NSEGCK,
     +                      NSS, ichk, 1, Nsol)
      END IF
C
...
Cdep  Revised to change ib loop counter
          ...
          DO i = ib, numinst
            ...
            ichk = 0
            CALL SGWF2SFR7RDSEG(nlst, lb, In, Iunitgwt, idum, 
     +                          1, ichk, 1, Nsol)
...
C     ******************************************************************
     ...
     +                        LAYHDT
      ...
      USE GWFSUBMODULE, ONLY: SUBLNK,LPFLNK,DVZ,DVZC                    !wschmid added 
      USE ICHKSTRBOT_MODULE
      IMPLICIT NONE
C     ------------------------------------------------------------------
C     SPECIFICATIONS:
...
C     ARGUMENTS
C     ------------------------------------------------------------------
      INTEGER Kkper, Kkstp, In, Iunitgwt, Iunitlak, IUNITSUB, Nsol
...
C       AND ADJUST TO MINIMUM SLOPE
                IF(Strm(2, irch).LT.0D0) THEN
                  WRITE(IOUT,9027) irch,strm(2,irch),strm(3,irch)
 9027             FORMAT(1X,
     ...
     +            ' AT DOWNSTREAM END OF SEGMENT!',I5,2F20.10)
                  STOP
                END IF
                IF( Strm(2, irch ).LT.1.0E-4) THEN
                  WRITE(IOUT,9028)  Istrm(4,irch), Istrm(5,irch),
     +                              Strm(2, irch)
 9028             FORMAT(1X,'SLOPE FOR SEGMENT AND REACH ',2(1x,I5),
     +                   ' IS WITH ',E10.3,' LESS THAN 1.0E-4:',
     +                   ' SETTING SLOPE TO 1.0E-4 ')
                  Strm(2, irch) = 1.0E-4
                END IF
              END IF
            END DO
...
C     *****************************************************************
      USE GWFSFRMODULE
      USE GLOBAL,       ONLY: IOUT,BOTM,LBOTM
c      USE GWFBASMODULE, ONLY: DVZ
      USE GWFSUBMODULE, ONLY: DVZ,SUBLNK,LPFLNK,DVZC
      IMPLICIT NONE
C     -----------------------------------------------------------------
C     SPECIFICATIONS:
C     -----------------------------------------------------------------
...
C1------SET POINTERS FOR CURRENT GRID.
      CALL SGWF2SFR7PNT(Igrid)
      fminslope = 1.0d-4 !--> small fminslope is a big problem for mannings: produces huge depths when sitting in the denominator of depth equation.
...
C
  ...
  111       format('***Warning*** Subsidence has caused a stream reach '
     ...
     +        ' and reach ',i5,/,' Resetting slope to ',PG15.8)
          ENDIF
        ENDDO
      DEALLOCATE(STRTOP)
      END SUBROUTINE 
...
C     *****************************************************************
     ...
     +                        RHS, BOTM, LBOTM, DELR, DELC
      ...
      USE LGRMODULE,    ONLY: ISCHILD                                   !swm - LGR added by wschmid
      IMPLICIT NONE
...
C-------SUBROUTINE GWF2SFR7BD
     ...
     +                    Iunituzf, Nsol, Ilgr, Lgriter, Ngrids, Igrid)  !cjm (added Iunitrch)
C     *****************************************************************
...
C-------SUBROUTINE SGWF2SFR7RDSEG
      SUBROUTINE SGWF2SFR7RDSEG(Nlst, Lstbeg, In, Iunitgwt,  
...
C
!     -----------------------------------------------------------------
      SUBROUTINE GWF2SFR7LAKE(Iunitlak)
C     ******************************************************************
