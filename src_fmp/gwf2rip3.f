      Module GWFRIPMODULE
        ...
        Integer, Save, Dimension(:),Pointer   :: NuSeg,NPoly
        Integer, Save, Dimension(:,:), Pointer:: CLoc
        Real, Save, Dimension(:), Pointer     :: Sxd,Ard,Rmax,Rsxd,RIPET
        Real, Save, Dimension(:,:), Pointer   :: fdh,fdr,HSurf 
        Real, Save, Dimension(:,:,:), Pointer :: CovPFSG
        Double Precision, Dimension(:,:,:), Pointer    :: C1,C2 
        Character(LEN=24), Save, Dimension(:), Pointer :: RIPNM    
        TYPE GWFRIPTYPE
          Integer,  Pointer      :: MAXRIP,IRIPCB,IRIPCB1,NRIPCL
          Integer,  Pointer      :: MAXPOLY,MAXTS,MXSEG
          Integer,  Dimension(:), Pointer     :: NuSeg,NPoly
          Integer, Dimension(:,:), Pointer    :: CLoc
          Real, Dimension(:), Pointer         :: Sxd,Ard,Rmax,Rsxd,RIPET
          Real, Dimension(:,:), Pointer       :: fdh,fdr,HSurf
          Real, Dimension(:,:,:), Pointer     :: CovPFSG
          Double Precision, Dimension(:,:,:), Pointer   :: C1,C2   
          Character(LEN=24),  Dimension(:), Pointer     :: RIPNM    
        END TYPE
...
!     ************************************************************************* 
      Subroutine GWF2RIP3AR(IN,IGRID)        
!     ************************************************************************* 
!     This subroutine allocates storage for all RIP-ET data, and reads in 
!     dimension variables that remain constant over the simulation
!     The follow variables are used:
...
!     CovPFSG.
! 
      Allocate(Sxd(MAXTS),Ard(MAXTS),Rmax(MAXTS),Rsxd(MAXTS),
     & RIPET(MAXRIP)) 
...
!     ET flux at the saturation extinction depth, and the number of  
!     segments used to interpolate the ET flux curve. 
!
      Do ITS=1, MAXTS
        If(IFREFM .EQ. 0) then
...
!    depth segment factors and ET flux segment factors.
!
        If(IFREFM .EQ. 0) then
...
!      ET flux at saturation extinction depth.
!
      Write(IOUT,170)
...
180     Format(1x,A,T27,F10.4,T38,F10.4,T52,E11.4,T65,E11.4)
      End do
!
!12---Print out plant functional subgroup name and active root depth and ET 
!     flux segments.
!
      Write(IOUT,190)
...
210     Format(1x,T27,A,10F10.4)
      End Do	  
!
!13---Save pointers to data
!
      Call SGWF2RIP3PSV(IGRID)        
!
!14---Return
!
      Return
      End Subroutine GWF2RIP3AR  
!
!     *************************************************************************
      Subroutine GWF2RIP3RP(IN,IGRID)
!     *************************************************************************
...
!
        write(IOUT,100)
100		Format(1x,/1x,'REUSING RIPARIAN CELL DATA FROM LAST STRESS PERIOD')
        Return
      Else
!
...
!
        Do NP =1,NPOLY(NC)	  	
          If(IFREFM .EQ. 0) then		   
            Read(IN,'(F10.2,25F10.5)')
     &      HSURF(NC,NP),(COVPFSG(NC,NP,I),I=1,MAXTS)
          Else
...
!
      Return
      End Subroutine GWF2RIP3RP 
!
!     *************************************************************************
      Subroutine GWF2RIP3FM(IGRID) 
!     *************************************************************************
...
!     *************************************************************************
      ...
      USE GWFRIPMODULE, Only:MAXTS,MXSEG,NRIPCL,Sxd,Ard,Rmax,Rsxd,fdH,
     &                       fdR,NuSeg,CLoc,NPoly,HSurf,CovPFSG,C1,C2
      Integer::IGRID				
      Integer::IL,IR,IC 
...
!
              IF(HH .GT. HK(KS) .and. HH .LE. HK(KS+1)) Then
!
...
!
      Return																																																																																																																																																																																																																																																																																																																					
! 
      End Subroutine GWF2RIP3FM
!
!     *************************************************************************
      Subroutine GWF2RIP3BD(KSTP,KPER,IGRID)
!     *************************************************************************
...
!
      IBD=0 
      If(IRIPCB .LT. 0 .and. ICBCFL .NE. 0) IBD=-1
      If(IRIPCB .GT. 0) IBD=ICBCFL
!
...
!
      ...
      End do
!
!6----Loop through each riparian cell if NRIPCL > 0.  
!       
      If(NRIPCL .EQ. 0) Return
      Do LC = 1, NRIPCL
!
!7----Get row, column, layer, and surface elevation,
!
        IL=CLoc(LC,1)
        IR=CLoc(LC,2)
        IC=CLoc(LC,3)
!
!8----Set HH=HNEW, and initialize RATE to zero 
!
        HH=HNEW(IC,IR,IL)
        RATE=zero
!
!9----Loop through the polygons
!
        Do LP=1,NPoly(LC)
!
!10---Loop through plant functional subgroups, calculate ETR and 
...
!
        ...
        End do  !Poly loop
!
!11---Load RATE to budget accumulator and to BUFF and RIPET
!
        RATOUT=RATOUT-RATE
        BUFF(IC,IR,IL)=BUFF(IC,IR,IL)+RATE
        RIPET(LC)=RATE  
!
!12---If saving cell-by-cell flows to list, Write flow.
!
        IF(IBD.EQ.2) CALL UBDSVB(IRIPCB,NCOL,NROW,IC,IR,IL,RATE,
     &                  AUXVAL,1,0,1,IBOUND,NLAY)
!
      End do	!Cell loop	
!
!13---Calculate the total ET for each PFSG.
!
      Do NTS=1,MAXTS
        Sum=0.0
        Do LC=1,NRIPCL 
      ...
      End Do
!
!14---Print locations, polygon number, HSurf, ET rate by plant functional
...
!
!15---Write out ET rates list file.
!
        Write(IOUT,120)(RIPNM(II), II=1,MAXTS)
120		format(1x,/,1x,' RIPARIAN ET',/1x,
     &         'LAYER  ROW  COLUMN     HEAD       CELL      ',
     &         'POLYGON   SURFACE                     CELL ET RATE BY ',
     &         'PLANT FUNCTIONAL GROUP',/1x,
     &         '                                  ET RATE   NUMBER',
     &         '    ELEVATION       ',20(2x,A24),/)
        DO LC=1, NRIPCL
          IL=CLoc(LC,1)
...
150     Format(/,1x,32x,'Total ET per PFSG',21x,20(2x,F10.5,14x))
      End If
!
!16---Check to see if cell-by-cell flows are to be saved as three-
!      dimensional array.
!
      If(IBD .EQ. 1) Then
      ...
      End if
!
!17---Check to see if cell location, polygon, head, surface elevation, and 
!     plant functional subgroup ET rates are to b saved as a formatted file.
!
      If(IRIPCB1.GT.0) Then
        DO LC=1,NRIPCL
          IL=CLoc(LC,1)
...
160         Format(4I5,2F10.2,25F10.5)
      ...
      End If
!
!18---Move total riparian loss into VBVL for printing in BAS1OT.
!
      VBVL(3,MSUM)=Zero
      VBVL(4,MSUM)=RATOUT
!
!19---Add riparian ET (riparian ET rate times time's step length)
!     to VBVL.
!
      VBVL(2,MSUM)=VBVL(2,MSUM)+RATOUT*DELT
!
!20---Move budget term labels to VBNM for printing by module BAS1OT.
!
      VBNM(MSUM)=TEXT
!
!21---Increment Budget term counter.
!
      MSUM=MSUM+1
!
!22---Return
...
!
!     *************************************************************************
      Subroutine GWF2RIP3DA(IGRID)
!     *************************************************************************
!     Deallocate RIP-ET memory
!     *************************************************************************
      USE GWFRIPMODULE 
!     *************************************************************************
!
...
!
      Return
!
      End Subroutine GWF2RIP3DA
!
!
!     *************************************************************************
      Subroutine SGWF2RIP3PNT(IGRID)
!     *************************************************************************
!     Set pointers to RIP-ET data for grid.
!     *************************************************************************
      USE GWFRIPMODULE 
!     *************************************************************************
...
!
      Return
!     
      End Subroutine SGWF2RIP3PNT      
      
!     *************************************************************************
      Subroutine SGWF2RIP3PSV(IGRID)      
!     *************************************************************************
