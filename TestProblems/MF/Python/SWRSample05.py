...
rows = datasize[0]
cols = datasize[1]
csr17x = zeros(rows, float)
csr17z = zeros(rows, float)
...
rows = datasize[0]
cols = datasize[1]
csr18x = zeros(rows, float)
csr18z = zeros(rows, float)
#--parse cross-section data
for i in range(0,rows):
...
width = 0.5
#pltb.bar(swr_t, swr_q05, width=width, align='center', color='m', linewidth=0, label='Observation location 5')
#pltb.bar(swr_t+width, swr_q07, width=width, align='center', color='g', linewidth=0, label='Observation location 7')
#pltb.plot(swr_t, swr_q05, 'm', linewidth=1, label='Observation location 5')
#pltb.plot(swr_t, swr_q06, 'b', linewidth=1, label='Observation location 6')
#pltb.plot(swr_t, swr_q07, 'g', linewidth=1, label='Observation location 7')
...
#pltb.set_xlim(xr)
pltb.set_xlim(0,350)
#pltb.set_xlim(200,230)
pltb.set_ylim(0,8.0)
i = set_sizexaxis(pltb,'%16.6g',ticksize)
i = set_sizeyaxis(pltb,'%16.6g',ticksize)
#-legend
#leg = pltb.legend(('Observation location 5', 'Observation location 6', 'Observation location 7', 'Observation location 8'), loc='upper right',ncol=1,labelspacing=0.25,columnspacing=1,handletextpad=0.5,handlelength=0.5,numpoints=1)
...
#leg = pltc.legend(('Observation location 1'), loc='upper right',ncol=1,labelspacing=0.25,columnspacing=1,handletextpad=0.5,handlelength=0.5,numpoints=1)
leg._drawFrame=False 
...
#pltd.set_xlim(200,230)
#pltd.set_xlim(xr)
pltd.set_xlim(0,350)
pltd.set_ylim(-750,1000)
#pltd.set_ylim(-400,100)
...
#leg = pltd.legend(loc='upper right',ncol=1,labelspacing=0.25,columnspacing=1,handletextpad=0.5,handlelength=0.5,numpoints=1)
leg._drawFrame=False 

#--runoff and flow duration
fig2 = pylab.figure(figsize=(8, 4), facecolor='w')
...
#ax.plot(swr_t, swr_bf, 'g', linewidth=1, label='Total Baseflow')
#ax.set_xlim(xr)
ax.set_xlim(0,350)
#ax.set_ylim(0.0,3.5)
text(0.025,0.85,'A',transform = ax.transAxes,size=12)
...
text(0.025,0.85,'B',transform = ax.transAxes,size=12)
...
fig3 = pylab.figure(figsize=(8, 6), facecolor='w')
fig3.subplots_adjust(wspace=0.25,hspace=0.25,left=0.075,right=0.975,bottom=0.075,top=0.975)
ax = fig3.add_subplot(311)
#plt2.plot(rain_t, rain_r01, 'b', linewidth=1, label='Rainfall zone 1')
#plt2.plot(rain_t, rain_r02, 'r', linewidth=1, label='Rainfall zone 2')
width = 1.0
...
ax.set_xlim(0,350)
ax.set_ylim(1.5,1.7)
text(0.025,0.85,'C',transform = ax.transAxes,size=12)
ax.set_xlabel('Time, days')	
ax.set_ylabel('Boundary stage, m')
i = set_sizexaxis(ax,'%16.6g',ticksize)
i = set_sizeyaxis(ax,'%16.6g',ticksize)
##-legend
#leg = ax.legend(loc='upper left',ncol=1,labelspacing=0.25,columnspacing=1,handletextpad=0.5,handlelength=2.0,numpoints=1)
#leg._drawFrame=False 

fig4 = pylab.figure(figsize=(8, 4.00), facecolor='w')
