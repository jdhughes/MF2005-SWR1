...
#--SWR results
SWR_file = '..\\SWRSample02\\Results.02\\SWRSample02_Stage.stg'
...
infile = '..\\SWRSample02\\SWIFT2D\\Processed\\c7r6_stage.02.csv'
swift2d = np.genfromtxt(infile,delimiter=',')
datasize = swift2d.shape
rows = datasize[0] - 1
cols = datasize[1]
swift2d_s61 = zeros(rows, float)
#--parse s61 data
for i in range(0,rows):
	ipos = i + 1
	swift2d_s61[i] = float( swift2d[ipos,3] )

infile = '..\\SWRSample02\\SWIFT2D\\Processed\\c10r6_stage.02.csv'
swift2d = np.genfromtxt(infile,delimiter=',')
datasize = swift2d.shape
rows = datasize[0] - 1
cols = datasize[1]
swift2d_s64 = zeros(rows, float)
#--parse s64 data
for i in range(0,rows):
	ipos = i + 1
...
#--save figures
outfigpng = '..\Figures\SWR01Sample02.02.png'
fig.savefig(outfigpng,dpi=300)
print 'created...', outfigpng


##--save data to file
#datafile = '..\\SWRSample02\\Results.02\\StageResults.dat'
#f = open(datafile, 'w')
#f.write('TIME           ,S57            ,S61            ,S64\n')
#for i in range(0,nt):
#    ctxt = '{0:15.7g},{1:15.7g},{2:15.7g},{3:15.7g}\n'.format( swr_t[i], swift2d_s57[i], swift2d_s61[i], swift2d_s64[i])
#    f.write( ctxt )
#f.close()
