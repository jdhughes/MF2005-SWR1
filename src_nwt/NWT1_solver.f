...
C3B-----GET OPTIONS.
   ...
   26    FORMAT(1X,'SPECIFIED OPTION:',/,1X,'SOLVER INPUT',
     1 ' VALUES ARE SPECIFIED BY USER')
      ELSE IF (LINE(ISTART:ISTOP).EQ.'CONTINUE') THEN
         irdopt = 1
         ICNVGFLG = 1
         WRITE(IOUT,27)
   27    FORMAT(1X,'SPECIFIED OPTION:',/,1X,'SOLVER WILL',
     1 ' CONTINUE IF NONCONVERGENCE OCCURS DURING SIMULATION')
      END IF
      IF ( irdopt.NE.0 ) GOTO 20
      LLOC = LLOCSAVE
      !LLOCSAVE = LLOC
      !CALL URWORD(LINE,LLOC,ISTART,ISTOP,1,I,R,IOUT,IN)
