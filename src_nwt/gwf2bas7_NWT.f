...
C  this file to use the IMPLICIT NONE statement.
      ...
      MODULE GWFBASMODULE
        ...
        INTEGER, SAVE, POINTER  ::IPRTIM,IPEROC,ITSOC,ICHFLG,ICNTHED
      ...
      TYPE GWFBASTYPE
        ...
        INTEGER, POINTER  ::IPRTIM,IPEROC,ITSOC,ICHFLG,ICNTHED
...
C     ------------------------------------------------------------------
     ...
     3                      ICNTHED,
     4                      IDDREF,IDDREFNEW,DELT,PERTIM,TOTIM,HNOFLO,
     5                      HDRY,STOPER,CHEDFM,CDDNFM,CBOUFM,VBVL,VBNM
C
...
C
     ...
     1         ICHFLG,ICNTHED,IDDREF,IDDREFNEW)
   ...
   26    FORMAT (1X,'THE FREE FORMAT OPTION HAS BEEN SELECTED')
    ...
    8    FORMAT(1X,'When solver convergence criteria are not met,',/
     ...
     2    1X,'discrepancy is greater than:',F10.4)
      ELSE IF(LINE(ISTART:ISTOP).EQ.'CONTHEADOUT') THEN
         ICNTHED = 1
         WRITE(IOUT,9)
    9    FORMAT(1X,'THE CONTINUOUS HEAD OUTPUT OPTION',
     1          1X,'HAS BEEN SELECTED')
      END IF
...
C     ------------------------------------------------------------------
     ...
     1                      IBOUND,IOUT,
     2                      BOTM,LBOTM     
      USE GWFBASMODULE,ONLY:PERTIM,TOTIM,HNOFLO,HDRY,IHEDFM,IHEDUN,
     2                      LBHDSV,ICNTHED,CHEDFM,IOFLG
C
      CHARACTER*16 TEXT
      REAL V
      DOUBLEPRECISION :: DSMALL,DHNOFLO,DHDRY,H,D,DB
      DATA TEXT /'            HEAD'/
C     ------------------------------------------------------------------
C
...
C3------MOVE HNEW TO BUFF FOR THE LAYER.
   ...
   59 CONTINUE
C
C-------ADJUST BUFF TO ELIMINATE HDRY VALUES IF ICNTHED EQUALS 1
C       THE HEAD IN THE NEXT ACTIVE UNDERLYING CELL OR THE BOTTOM OF
C       THE MODEL IS USED AND WRITTEN TO THE HEAD SAVE AND/OR THE
C       LISTING FILE
      IF (ICNTHED.EQ.1) THEN
        !DSMALL = EPSILON(1.0D-00)
        DSMALL = 1.0D-06
        ...
        DO K=1,NLAY
          KL=K
          IF(IXSEC.NE.0) KL=1
          IF(IOFLG(KL,1).EQ.0 .AND. IOFLG(KL,3).EQ.0) CYCLE
          DO I=1,NROW
            DO J=1,NCOL
C               DETERMINE IF THIS LAYER IS DRY
C               IF LAYER IS WET EVALUATE NEXT COLUMN
              H = HNEW(J,I,K)
              D = ABS( H - DHDRY ) 
              IF ( D.GT.DSMALL ) CYCLE
C               EVALUATE THE UNDERLYING LAYERS
              KK = K
              IF ( K.LT.NLAY ) KK = K + 1
              DO KP=KK,NLAY
                V = BOTM(J,I,LBOTM(KP))
                H = HNEW(J,I,KP)
C                 DETERMINE IF THIS LAYER IS DRY
C                 IF DRY GO TO THE NEXT LAYER
                D = ABS( H - DHDRY ) 
                IF ( D.LT.DSMALL ) CYCLE
C                 STOP IF THIS LAYER IS INACTIVE
C                 SET BUFF TO BOTTOM OF THE OVERLYING LAYER
                D = ABS( H - DHNOFLO )
                IF ( D.LT.DSMALL ) THEN
                  V = BOTM(J,I,LBOTM(KP-1))
                  EXIT
                END IF
C                 UNDERLYING LAYER IS ACTIVE - SET BUFF TO THIS VALUE
                V = REAL( H, 4 )
                EXIT
              END DO
              BUFF(J,I,K) = V
...
C
        ...
        DEALLOCATE(GWFBASDAT(IGRID)%ICNTHED)
...
C
        ...
        ICNTHED=>GWFBASDAT(IGRID)%ICNTHED
...
C
        ...
        GWFBASDAT(IGRID)%ICNTHED=>ICNTHED
