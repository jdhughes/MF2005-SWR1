C
C     VERSION 0.56.0
C       - IMPLEMENTED OPTION TO ALLOW OUTPUT OF COMPACT BUDGET DATA
C       - ADDED SUPPORT FOR AUXILIARY VARIABLES
C       - MODIFIED SSWR_P_RGFLOW SO ASCII OUTPUT IS VERTICALLY STACKED FOR
C         EACH REACH GROUP. THIS IS THE SAME FORMAT USED FOR ALL OTHER ASCII
C         OUTPUT EXCEPT FOR STAGE.
C
C     VERSION 0.55.0
C       - REDUCED MEMORY FOOT PRINT FOR SWR1 STORAGE BY REDUCING DATA STORAGE FOR 
C         SAVING SWR1 RESULTS.  RECALCULATING RESULTS FROM STAGE AND GATE OPENING DATA
C         SAVED FOR EACH SWR1 TIME STEP.
C       - MODIFIED BINARY OUTPUT FOR QAQ TERMS AND STRUCTURE FLOW SO THAT INTEGER
C         ARRAY, DIMENSIONED TO NREACHES, IS WRITTEN PRIOR TO STRESS PERIOD AND TIME
C         STEP INFORMATION.  PREVIOUSLY THIS INFORMATION WAS WRITTEN INSTREAM PRIOR TO
C         THE DATA FOR EACH REACH.  NOW IT POSSIBLE TO CALCULATE HOW MUCH DATA IS
C         AVAILABLE FOR EACH PRINT TIME PRIOR TO READING THE DATA.  THIS MODIFICATION
C         WILL BE USEFUL FOR PYTHON AND OTHER BINARY READERS FOR CREATING A CATALOG
C         OF AVAILABLE BINARY QAQ AND STRUCTURE FLOW DATA.
C
C     VERSION 0.54.0
C       - ADDED OPTION FOR USING BILINEAR INTERPOLATION TO CALCULATE THE COORDINATE-
C         INVARIANT QM FOR TWO-DIMENSIONAL MODEL-BASED REACHES.  THIS IS THE DEFAULT
C         APPROACH USED.  THE ORIGINAL SOURCE CELL APPROACH IS IMPLEMENTED USING
C         ISWROPT "USE_SOURCECELL_INVARIATE_QM" IN DATASET 1B.
C       - MODIFIED GSOL TO ACCOUNT FOR SCALING WHEN CALCULATING THE MAXIMUM
C         DELTAX, RESIDUAL, AND/OR L2NORM FOR COMPARISON TO TOLS AND TOLR
C       - FIXED BUG IN FIXED WEIRS WHEN THE STRUCTURE INVERT IS MORE THAN TEN (10)
C         LENGTH UNITS BELOW THE WATER SURFACE ELEVATION.  MODIFIED STRTOP FOR
C         FIXED WEIRS SO THAT THEY ARE ALWAYS ONE HUNDRED (100) LENGTH UNITS ABOVE
C         THE MAXIMUM OF THE HEADWATER AND TAILWATER STAGE - THIS SHOULD BE 
C         SUFFICIENT FOR ALL APPLICATIONS OF SWR1
C
C     VERSION 0.53.0
C       - MADE THE COORDINATE-INVARIANT QM FOR TWO-DIMENSIONAL MODEL-BASED REACHES
C         THE DEFAULT CONDITION. MODIFIED ISWROPT "USE_COORDINATE_INVARIATE_QM" 
C         SPECIFIED IN DATASET 1B TO "USE_ORIGINAL_2D_QM_FORMULATION" TO ALLOW
C         ORIGINAL FORMULATION TO BE USED. AN EXPLICIT FORMULATION OF THE MAGNITUDE
C         OF QM IS USED TO CALCULATE A COORDINATE-INVARIANT FORM OF THE DIFFUSIVE-WAVE
C         APPROXIMATION BY DEFAULT (THE MAGNITUDE OF QM IS UPDATED EACH OUTER ITERATION).
C         CAN ALSO IMPLICITLY CALCULATE THE MAGNITUDE OF QM USING ISWROPT
C         "USE_IMPLICIT_INVARIATE_QM".  THE EXPLICIT FORMULATION IS GENERALLY GOOD ENOUGH
C         AND IS SIGNIFICANTLY FASTER THAN THE IMPLICIT FORMULATION.  THE IMPLICIT
C         FORMULATION SHOULD ALWAYS RESULT IN THE "BEST" ANSWER.
C       - ADDED OPTION TO OUTPUT A MODFLOW RIV PACKAGE AT RUN TIME. THIS OPTION IS
C         IMPLEMENTED USING ISWROPT "SAVE_RIVER_PACKAGE" SPECIFIED IN DATASET 1B.
C         MODFLOW-2005 RIV PACKAGE DATA IS OUTPUT AT EACH SWR1 OUTPUT PRINT TIME.
C         ALSO ADDED ISWROPT "SAVE_RIVER_PACKAGE_ALL" TO SAVE ALL SWR REACHES,
C         INCLUDING INACTIVE REACHES, A CONDUCTANCE OF ZERO IS USED FOR ALL
C         INACTIVE REACHES.
C
C     VERSION 0.52.0
C       - ADDED OPTION TO CALCULATE COORDINATE-INVARIANT QM FOR TWO-DIMENSIONAL 
C         MODEL-BASED REACHES. IMPLEMENTED USING ISWROPT "USE_COORDINATE_INVARIATE_QM" 
C         SPECIFIED IN DATASET 1B.
C       - ADDED OPETION TO OFFSET AND SCALE TOTTIME VALUES FOR TABULAR DATA SPECIFIED
C         IN DATASET 4D.
C
C     VERSION 0.51.0
C       - ADDED UPSTREAM WEIGHTING OPTION BACK IN (ISWRUPWT). 
C         WHAT IS GOING ON WITH THIS CODE?? ADDED FOR LEVEL AND TILTED V-CATCHMENT
C         PROBLEM (SWRTESTSIMULATION16)
C       - DO NOT CALCULATE FLOW IF PERIMETER IS EQUAL TO DZERO IN SSWR_CALC_UFLOW.
C         REVISED FROM EVALUATION OF THE PERIMETER BEING LESS THAN NEARZERO.
C       - REVISED SSWR_KELLEY_PERTB TO ALWAYS USE POSITIVE PERTURBATION.
C         PREVIOUSLY NEGATIVE STAGE VALUES RESULTED IN NEGATIVE PERTURBATIONS.
C         THIS CAUSED PROBLEMS FOR NEGATIVE STAGE VALUES AND WAS DETERMINED 
C         WHEN DEVELOPING THE LEVEL V-CATCHMENT PROBLEM (SWRTESTSIMULATION16)
C         WHICH HAS NEGATIVE STAGE VALUES IN THE ONE-DIMENSIONAL CHANNEL 
C         CONNECTED TO THE LEVEL FLOW PLANE,
C       - REVISED NEARZERO TO BE EPSILON(1.0D0) INSTEAD OF 1.0D-20. THIS IS 
C         APPROXIMATELY 1.0D-16.
C       - MODIFIED AREA USED TO CALCULATE RAINFALL APPLIED TO A REACH (QPR). PREVIOUSLY
C         THE WATER SURFACE AREA WAS USED TO CALCULATE QPR AS THE PRODUCT OF
C         THE SPECIFIED RAINFALL RATE AND THE WATER SURFACE AREA. THIS APPROACH
C         WAS MODIFIED TO USE THE MAXIMUM REACH SURFACE AREA.  THIS VALUE IS CONSTANT
C         AND IS BASED ON THE ASSUMPTION THAT ALL RAINFALL APPLIED TO THIS SURFACE AREA
C         IS ACCUMULATED IN THE WETTED PORTION OF THE REACH. THE RAINFALL AREA IS
C         THE MINIMUM OF THE MAXIMUM SURFACE AREA IN THE PROCESSED GEO DATA AND THE
C         MODFLOW CELL AREA AND IS SAVED IN THE DERIVED REACH TYPE (RAINAREA).
C
C     VERSION 0.50.0
C       - MODIFIED QAQ CONVERGENCE CHECK TO EXPLICITLY EVALUATE THE DIFFERENCE
C         BETWEEN SWR1 AND MODFLOW RATHER THAN THE CHANGE BETWEEN SWR1
C         QAQ CALCULATIONS BETWEEN SUBSEQUENT NON-LINEAR (OUTER) ITERATIONS.
C         ALSO CONVERTED TOLA FROM AN ABSOLUTE TO A RELATIVE (FRACTIONAL)
C         TOLERANCE. THIS GIVES FINER CONTROL ON THE AGREEMENT BETWEEN SWR1
C         AND MODFLOW QAQ CALCULATIONS
C
C     VERSION 0.49.0
C       - CONVERTED SWROPTIONS KEYWORD IN DATASET 1A TO ISWROPT.
C         IF ISWROPT IS NON-ZERO - DATASET 1B IS SPECIFIED.
C         IF ISWROPT IS ZERO     - DATASET 1B IS NOT SPECIFIED AND ALL
C                                  DEFAULT SWR1 OPTIONS ARE USED
C       - MOVE SPECIFICATION OF ALL NON-DEFAULT OPTIONS TO DATASET 1B.
C         EXAMPLES INCLUDE THE NON-COVERGENCE CONTINUATION OPTION, INEXACT
C         NEWTON APPROACH, ETC.
C       - SIMPLE DIAGONAL SCALING HAS BEEN ELIMINATED AND REPLACED BY
C         L2NORM SCALING.  RELYING ON GENERIC A, X, AND B SCALING SUBROUTINE
C         IN GSOL7.F (GSOL_SCALE)
C       - ADDED SPARSKIT2 ILUT PRECONDITIONER (ilut SUBROUTINE) AND TWO
C         ADDITIONAL SUBROUTINES (lusol AND qsplit) TO GSOL7.F
C       - INCLUDED GNU LICENSE INFORMATION FOR REVERSE CUTHILL MCKEE AND 
C         SPARSKIT2 LIBRARIES IN GSOL7.F
C       - ADDED ISWRPSTR VARIABLE TO DATASET 1A TO DEFINE WHETHER STRUCTURE
C         OUTPUT WILL BE WRITTEN TO AN EXTERNAL ASCII OF BINARY FILE AND THE 
C         UNIT NUMBER OF THE EXTERNAL FILE.  THIS USED TO BE SPECIFIED USING
C         SWR1 OPTIONAL VARIABLES IN DATASET 1B.  MODIFIED TO BE CONSISTENT
C         WITH THE FORMAT FOR SPECIFYING OTHER STANDARD SWR1 OUTPUT DATA.
C
C     VERSION 0.48.0
C       - ADDED REORDERING FOR SOLUTION - REVERSE CUTHILL MCKEE ORDERING
C
C     VERSION 0.47.0
C       - ADDED ILUT PRECONDITIONER - BASED ON SPARSKIT2
C         *** HACKED IN - WILL TAILOR TO SWR1 IF THERE ARE SIGNIFICANT BENEFITS ***
C       - MODIFIED ADAPTIVE TIME-STEPPING ALGORITHM TO ENSURE THAN PRINT TIME
C         AND END OF MODFLOW STRESS PERIOD TIMES ARE HIT.  MADE SURE THAT PRINT
C         FLAGS AND END OF MODFLOW TIME STEP FLAGS ARE RESET IF CONVERGENCE
C         CRITERIA OR ADAPTIVE STAGE AND FLOW CRITERIA ARE VIOLATED - THIS PREVENTS
C         PRINT DATA BEING OUTPUT TO SWR1 OUTPUT FILES AT NON-PRINT TIMES.
C       - FIXED APPARENT BUG IN SWR1 STAGE USED FOR THE FIRST SWR1 TIME STEP IN A
C         MODFLOW TIME STEP WHEN MULTIPLE SWR1 TIME STEPS ARE SIMULATED DURING A
C         SINGLE MODFLOW TIME STEP.
C       - MODIFIED ADAPTIVE TIME STEP ALGORITHM TIME STEP REDUCTION IF SWR1 TIME STEP
C         DOES NOT CONVERGE TO REDUCE USING RTMULT INSTEAD OF THE RATIO OF THE MAXIMUM
C         RESIDUAL AND RTOL.  PREVIOUS APPROACH TENDED TO BE VERY AGGRESSIVE IN 
C         REDUCING THE SWR1 TIME STEP - TYPICALLY THE TIME STEP WAS REDUCED TO RTMIN.
C
C     VERSION 0.46.0
C       - MOVED OPTIONAL SWR1 PARAMETERS AND SETTING TO NEW DATASET 1B.  SPECIFYING
C         THE "SWROPTIONS" KEYWORD AT THE END OF DATASET 1A.  DATASET 1B IS TERMINATED
C         WITH THE KEYWORD "END".  NEED TO ADD SUMMARY OF KEYWORDS TO LISTING FILE.
C       - ADDED OPTION TO SOLVE LINEAR SYSTEM IN TERMS OF STAGE INSTEAD OF 
C         UPGRADE VECTOR (DX).  IMPLEMENTED USING SWR1 KEYWORD IN DATASET 1B.
C       - ADDED OPTION FOR SIMPLE DIAGONAL SCALING CONTROLLED BY ISOLVER 
C         VARIABLE.  ISOLVER VALUES OF 22 AND 23 USE SIMPLE DIAGONAL SCALING - 
C         ISOLVER USED BY MODEL CALCULATED BY SUBTRACTING 20. ISOLVER VALUES OF
C         11 AND 12 USE SCALING BASED ON PCG (HILL, 1990) - ISOLVER USED BY MODEL
C         CALCULATED BY SUBTRACTING 10.
C       - ADDED OPTION TO OUTPUT STRUCTURE GATE ELEVATION, GATE OPENING, AND
C         STRUCTURE DISCHARGE FOR EACH STRUCTURE. USING KEYWORD ARGUMENT IN 
C         DATASET 1B (PSWRSTR).
C       - ADDED OPTION TO OUTPUT AVERAGE RESULTS WITHIN A PRINT INCREMENT (RTPRN).
C         USING KEYWORD ARGUMENT IN DATASET 1B (PAVERAGE)
C
C     VERSION 0.45.0
C       - ADDED OPTION TO USE SWR1 STAGE RESULTS FROM A PREVIOUS RUN TO
C         DEFINE STAGES FOR CONSTANT STAGE REACHES.  IMPLEMENTED USING
C         TABULAR DATA FILE AND AN ALTERNATIVE TSDATA VARIABLE (STGRES)
C         THAT CONTAINS THE SWR STAGE DATA.
C       - ADDED ABILITY TO READ DATASET 11A, 13A-13B FROM AN EXTERNAL FILE
C         SPECIFIED IN THE NAME FILE (EXTERNAL KEYWORD) AND A TEMPORARILY
C         OPENED FILE (OPEN/CLOSE KEYWORD).
C
C     VERSION 0.44.0
C       - REVISED WEIR AND SPILLWAY EQUATION OF BARKAU (1996) TO ELIMINATE
C         DISCONTINUOUS FLOW TRANSITION BETWEEN UNSUBMERGED AND SUBMERGED
C         CONDITIONS.  REVISED WEIR AND SPILLWAY EQUATIONS RESULT IN COMPARABLE
C         FLOW RESPONSES TO GENERALIZED SPILLWAY EQUATION OF ANSAR AND 
C         CHEN (2009). THE ADVANTAGE OF THE REVISED WEIR AND SPILLWAY EQUATIONS
C         BARKAU (1996) IS STANDARD WEIR AND ORIFICE DISCHARGE COEFFICIENTS CAN
C         BE SPECIFIED AND OPPOSED TO C1, C2, AND C3 COEFFICIENTS WHICH CAN 
C         RESULT IN UNREASONABLE CALCULATED DISCHARGE COEFFICIENTS FOR CERTAIN
C         FLOW CONDITIONS (SEE ALVES AND MARTINS 2011).
C       - ADDED CHECKS TO SKIP SURFACE-WATER/GROUNDWATER EXCHANGE CALCULATIONS
C         FOR SURFACE WATER ONLY SIMULATIONS IN THE GWF2SWR7FM AND GWF2SWR7BD
C         SUBROUTINES.
C        
C     VERSION 0.43.0
C       - REVISED CALCULATION OF CONSTANT STAGE REACH GROUP FLOW TERM (QCRFLOW)
C         IS CALCULATED WHEN THE CONSTANT STAGE IS CHANGED DURING A SIMULATION.
C         THIS MODIFICATION WAS DONE TO ACCOMODATE CONSTANT STAGE CHANGES THAT
C         CAN OCCUR EACH SWR1 TIME STEP IF TABULAR DATA IS USED.
C       - ADDED THE ABILITY TO USE BOTH SIMULATED STAGES AND FLOWS TO CONTROL
C         STRUCTURE OPERATIONS.
C       - SIMPLIFIED THE SPECIFICATION OF OPERATION DATA FOR SWR STRUCTURES.
C         REVISED INPUT IS HUMAN READABLE.
C       - ADDED ABILITY TO READ TABULAR DATA TO SPECIFY RAINFALL, EVAPORATION,
C         STAGE, SWR TIMESTEP LENGTH, AND STRUCTURE DATA.  TABULAR DATA CAN BE
C         SPECIFIED AT A SMALLER FREQUENCY THAN MODFLOW STRESS PERIODS.  TABULAR
C         DATA CAN BE INTEGRATED OR INTERPOLATED OVER A SWR TIMESTEP.  ALTERNATIVELY
C         THE VALUE CLOSEST TO THE END OF THE SWR TIMESTEP CAN BE USED.
C       - ADDED ABILITY TO READ BREAKPOINT DATA FOR STRCRIT OR STRVAL FOR EACH 
C         OPERABLE STRUCTURE (ISTRTYPE=1,3,8,9,10). USING TABDATA KEYWORD TO
C         SPECIFY THIS OPTION.
C       - ADDED STRCRITC VARIABLE TO ALLOW A STRUCTURE TO BE CLOSED AT A DIFFERENT
C         ELEVATION THAN IT IS TURNED ON.  THIS VARIABLE IS AN OFFSET THAT IS ADDED
C         TO STRCRIT.
C       - ADDED GENERIC STRUCTURE (ISTRTYPE=10) BASED ON THE GENERALIZED SPILLWAY 
C         EQUATION OF ANSAR AND CHEN (J. HYDR. ENG. 2009).
C       - ADDED STRUCTURE TYPE (ISTRTYPE=11) TO PASS SFR2 DIVERSIONS AS INFLOWS TO
C         THE SWR PROCESS.
C       - ADDED ABILITY TO COUPLE UNCONNECTED STRUCTURES (ISTRCONN=0) TO THE SFR2
C         PACKAGE.  ALL STRUCTURES, EXCEPT ISTRTYPE=11, CAN DELIVER WATER BACK TO THE
C         SFR2 PACKAGE.  MULTIPLE SWR1 PROCESS STRUCTURES AT THE SAME SWR1 REACH
C         LOCATION CAN BE COUPLED TO THE SAME SFR2 SEGMENT. THE SFR2 SEGMENT RECEIVING 
C         THE SWR1 STRUCTURE FLOW SHOULD NOT HAVE AN UPSTREAM CONNECTION (I.E., THE
C         SFR2 SEGMENT SHOULD BE AN UNCONNECTED TRIBUTARY).
C       - ADDED EXTERNAL FLOW TERM (QEXTFLOW) TO CONTINUITY EQUATION AND SUBROUTINE
C         TO BE CALLED EXTERNAL TO THE SWR PROCESS (USING THE GWF2SWR7EX SUBROUTINE)
C         FOR FILLING QEXTFLOW FROM EXTERNAL CALCULATION ROUTINES. THIS ROUTINE COULD
C         BE CALLED FROM THE FARM PROCESS (FMP2).
C       - MODIFIED SPECIFIED DISCHARGE STRUCTURE TO USE SWR_CALC_STRVALS SUBROUTINE
C         TO ADJUST DISCHARGE RATES.
C
C     VERSION 0.42.0
C       - REVISED CONNECTIVITY - USERS MUST DEFINE FULL CONNECTIVITY IN DATASET 4B
C       - REMOVED ALL CODE FOR CALCULATING BIS AND CIT SYSTEM OF EQUATIONS
C
C     VERSION 0.41.0
C       - ADDED OPTION FOR OUTPUT OF CONNECTION FLOW AND VELOCITIES (ISWRPQM) TO
C         ASCII AND BINARY OUTPUT FILES - NOT FULLY IMPLEMENTED. 
C           NOTE: NEED TO REWORKLATERAL FLOW FUNCTION SO THAT STRUCTURES AND DIFFUSIVE
C                 WAVE LATERAL FLOW TERMS ARE CALCULATED WITHIN SAME FUNCTION CALL
C                 AND THAT BOUNDARY STRUCTURES ARE CALCULATED AS PART OF THE QCR TERMS.
C                 SOME WORK HAS BEEN DONE AS PART OF THIS VERSION - STRUCTURE CALL
C                 FOR INDIVIDUAL STRUCTURES HAS BEEN GENERALIZED TO A SEPARATE CALL
C                 THIS WILL BE FULLY IMPLEMENTED IN VERSION 0.42.0  
C       - ADDED ABILITY TO USE EXACT NEWTON CONDITION WITH KRYLOV SOLVERS (TOLR>0.0)
C       - ADDED ABILITY TO USE PREVIOUS STAGE TO OPEN AND CLOSE OPERABLE PUMPS, GATES,
C         AND WEIRS
C
C     VERSION 0.40.0
C       - CODE CLEAN-UP FOR FMP
C       - REIMPLEMENTED OPTION FOR
C           1. NO STORAGE CONSIDERATION FOR STEADY-STATE SIMULATIONS (ISWRSS = 1)
C              DAMPSS > 0.0
C           2. PSEUDO-TRANSIENT CONTINUATION APPROACH (ISWRSS = 2)
C              DAMPSS < 0.0
C       - ADDED DAMPENING TO ALL SOLVERS - CAN ALSO BE ACCELERATION
C         SEPARATE STEADY-STATE AND TRANSIENT DAMPENING FACTORS.
C       - ADDED SUMMARY OUTPUT OF SWR1 PRINT TIMES TO MODFLOW LIST FILE
C       - ADDED MODIFIED ILU0 - ROW SUM MODIFICATION OF DIAGONAL (IPC=3)
C
C     VERSION 0.39.0
C       - CONVERTED LINEAR SOLVERS TO GENERAL USE SOLVER
C       - CONSOLIDATED OUTER ITERATION (NON-LINEAR NEWTON STEP)
C         FOR ALL LINEAR SOLVERS (SSWR_SOLWRP)
C
C     VERSION 0.38.0
C       - ADDED CRITICAL DEPTH BOUNDARY CONDITION TO UNCONTROLLED GRADIENT 
C         CONNECTION STRUCTURE WHEN DOWNSTREAM CONNECTION IS 0 AND A USER-DEFINED
C         REACH BED ELEVATION IS NOT SPECIFIED IN INPUT ITEM 13A
C
C     VERSION 0.37.0
C       - ADDED USE OF SSWRLSTRD TO INPUT ITEM 4
C       - SLIGHT REVISION TO GMRES
C
C     VERSION 0.36.0
C       - RENAMED COMPUTATIONAL ELEMENTS TO REACH GROUPS
C       - ADDITIONAL ERROR CHECKING
C       - REMOVED GCNDFCT
C
C     VERSION 0.35.0
C       - REVISED STEADY-STATE APPROACH (AGAIN)
C         MOVED VOLUME CONVERGENCE FROM GWF2SWR7FM TO GWF2SWR7CV
C
C     VERSION 0.34.0
C************************************************************************************
C BEGIN FINALIZING OPTIONS TO BE INCLUDED IN VERSION 1 OF SWR1
C************************************************************************************
C       - LIMIT STEADY STATE OPTION TO PSEUDO-TRANSIENT CONTINUATION APPROACH
C         (ISWRSSFL=2) IMPLEMENTED IN VERSION 0.32.0. REMOVED ISWRSSFL VARIABLE     
C         FROM DATA INPUT
C       - REMOVED BTFCT AND BTRED FROM DATA INPUT SINCE BRENDT'S METHOD
C         IS USED TO FIND THE MINIMUM WHEN BACK TRACKING IS USED
C       - REMOVED ISWREXCH FROM DATA INPUT SINCE SWR-MODFLOW COUPLING ALWAYS
C         USE ISWREXCH>0 APPROACH (HCOF+RHS)
C       - REMOVED ABILITY TO SPECIFY ISWT AND ITWT.  SET AT DEFAULT VALUES SUCH THAT
C         SWR1 USES CENTRAL IN SPACE (ISWT=0) AND BACKWARD IN TIME (ITWT=1).  THESE
C         WEIGHTING APPROACHES ARE CONSISTENT WITH MODFLOW SPATIAL AND TEMPORAL 
C         WEIGHTING. BIS AND CIT FUNCTIONALITY WILL BE REMOVED FROM THE OFFICIAL
C         RELEASE.
C       - CONVERTED SWR1 TIMESTEP SPECIFICATION FROM MINIMUM AND MAXIMUM NUMBER OF
C         SWR1 TIMESTEPS TO MINIMUM (RTMIN) AND MAXIMUM (RTMAX) SWR1 TIMESTEPS
C       - ADDED ABILITY TO SPECIFY FREQUENCY OF SWR OUTPUT (RTPRN). THE SWR OUTPUT
C         FREQUENCY MUST BE EQUAL TO OR GREATER THAN RTMIN.  IF THE SWR OUTPUT
C         FREQUENCY IS LESS THAN RTMIN IT IS SET TO ZERO AND SWR OUTPUT IS SAVED
C         AT THE END OF EACH MODFLOW TIME STEP
C       - REMOVED ABILITY TO SPECIFY SSOR PRECONDITIONER (IPC=4) AND RELATED DSSOR
C         RELAXATION PARAMETER. SSOR PRECONDITIONING FUNCTIONALITY WILL BE REMOVED
C         FROM THE OFFICIAL RELEASE.
C       - ADDED ABILITY TO OUTPUT SUMMARY OF THE LOCATION AND MAGNITUDE OF THE 
C         MAXIMUM VELOCITY AND FROUDE NUMBER FOR EACH SWR TIME STEP AT THE END
C         OF A MODFLOW TIME STEP.  ACTIVATED BY SPECIFYING A ISWRPRFR VALUE GREATER
C         THAN ZERO IN INPUT ITEM 1
C************************************************************************************
C
C     VERSION 0.33.0
C       - ADDED ALTERNATIVE WEIGHTING OPTION THAT ALTERS HYDRAULIC 
C         GRADIENT CALCULATIONS WHEN THE STAGE IN ONE REACH GROUP
C         IS BELOW THE BOTTOM ELEVATION OF THE STAGE IN A CONNECTED
C         REACH GROUP
C
C     VERSION 0.32.0
C       - STEADY-STATE SOLUTION MODIFIED - INCLUDED OPTION FOR 1) EXCLUDING
C         VOLUME FROM CONTINUITY EQUATION, 2) CONSIDERING VOLUME IN CONTINUITY
C         EQUATION AND RUNNING UNTIL ABS(DV) IS LESS THAN TOLR, OR 3) USING
C         2) AND 1) ON ODD AND EVEN OUTER ITERATIONS, RESPECTIVELY.  
C         ADDED ISWRSSFL VARIABLE TO ALLOW USER TO SELECT
C       - FIXED ERROR IN ISTRTYPE FOR EXCESS VOLUME STRUCTURE AND FREE CONNECTION
C         STRUCTURE TYPE IN CASE SELECT IN FUNCTION CALCULATING STRUCTURE FLOWS
C       - MODIFIED FREE CONNECTION TYPE TO ALLOW SPECIFICATION OF INVERT ELEVATION,
C         USING AN ISTRUCTTYPE = -2.  FOR REACHES WITH NO END CONNECTION, USE OF 
C         THIS OPTION ALLOWS A ZERO-DEPTH GRADIENT STRUCTURE TO BE SPECIFIED.
C       - MODIFIED INPUT ITEM 2 TO ELIMINATE NEED TO SPECIFY DMAXRAI, DMAXSTG, AND
C         DMAXINF WHEN ADAPTIVE TIME-STEPPING IS NOT USED NTMIN=NTMAX AND RTMULT = 1.0
C       - ADDED MINIMIZATION APPROACH USING BRENT'S METHOD TO FIND SMALLEST L2NORM
C         WHEN BACKTRACKING IS USED.  ELIMINATES NEED FOR BTFCT AND BTRED PARAMETERS
C         (NOTE - THESE HAVE NOT BEEN REMOVED FROM DATA INPUT YET).
C
C     VERSION 0.31.0
C       - MODIFIED METEROLOGIC AND LATERAL INFLOW DATA SPECIFICATION TO ALLOW
C         SEPARATE SPECIFICATION OF METEROLOGIC AND LATERAL INFLOW DATA
C 
C     VERSION 0.30.0
C       - FULL IMPLEMENTATION OF INACTIVE REACHES AND REACH GROUPS
C         o ADDED CHECK FOR INACTIVE REACH GROUPS IN CONSTANT
C           STAGE FLUX ESTIMATION FUNCTION
C         o ADDED CHECK THAT ALL REACHES IN A REACH GROUP ARE INACTIVE
C           IF ONE IS INACTIVE.  THIS ENSURES A REACH GROUP IS INACTIVE
C           IF ANY REACH MEMBER OF THE REACH GROUP IS INACTIVE.
C
C     VERSION 0.29.0
C       - ADDED REMAINING STRUCTURES FROM TEST CODES
C         o CULVERT
C         o FIXED GATE SPILLWAY
C         o MOVABLE CREST WEIR
C         o OPERABLE GATED SPILLWAY 
C       - ADDED OPERATIONAL CONTROL TO SPECIFIED DISCHARGE STRUCTURE
C
C     VERSION 0.28.0
C       - ADDED CHANGES TO FAILURE CONDITION FOR BICGSTAB AMD GMRESM SOLVERS
C         WHEN ZERO ELEMENTS ON THE DIAGONAL ARE ENCOUNTERED BUT CURRENT
C         NUMBER OF SWR TIMESTEPS ARE LESS THAN THE MAXIMUM NUMBER OF SWR
C         TIMESTEPS.  SAME CHANGE AS CHANGES TO LU DECOMPOSITION IN VERSION 0.27.0
C       - ADDED LINEAR SCALING FUNCTIONALITY BETWEEN DUPDPTH AND DMINDPTH AS
C         ALTERNATIVE TO S-CURVE SCALING
C       - ADDED DMAXRAI TO ADJUST TIMESTEP BASED ON MAXIMUM SWR1 RAINFALL RATE
C         TO PREEMPTIVELY REDUCE TIMESTEP WHEN RAIN IN ANY REACH EXCEEDS DMAXRAI
C         AND ADAPTIVE TIMESTEP ALGORITHM IS BEING USED
C       - MODIFIED ALTERNATIVE WAYS OF INCORPORATING QAQFLOW TERMS IN RHS+HCOF
C         OR RHS ON ALTERNATING OUTER ITERATIONS, RHS ONLY, OR RHS+HCOF ONLY.
C         WATER BUDGET CALCULATIONS ALSO REFLECT OPTIONS FOR COUPLING QAQFLOW IN
C         GROUNDWATER FLOW EQUATION.  MODFLOW QAQFLOW IS RECALCULATED IF QAQFLOW
C         IS COUPLED USING RHS+HCOF.
C       - ADDED WATER BUDGET SUMMARY OF DISCREPANCY IN MODFLOW AND SWR1 QAQFLOW
C         TERMS TO ALLOW USERS TO ASSESS THE DISCREPANCY BETWEEN MODFLOW AND
C         SWR1 ESTIMATES OF QAQFLOW.  THIS WILL ALLOW USERS TO DECIDE IF QAQFLOW
C         CONVERGENCE SHOULD BE ENFORCED AND/OR TIGHTENED.
C       - FIXED BUG IN GWET
C       - FIXED BUG IN SCALING OF SWR1 EVAP WHEN BETWEEN DUPDPTH AND DMINDPTH
C
C     VERSION 0.27.0
C       - UPDATED CODE TO REFLECT CHANGES TO SWR DATA INPUT FORMAT
C         o 2D ARRAY READER FOR RAINFALL, EVAPORATION, LATERAL INFLOW, AND
C           REACH STAGES
C       - REVISED ADAPTIVE TIMESTEPPING AND FAILURE CONDITION FOR SOLVERS
C         WHEN ZERO ELEMENTS ON THE DIAGONAL ARE ENCOUNTERED BUT CURRENT
C         NUMBER OF SWR TIMESTEPS ARE LESS THAN THE MAXIMUM NUMBER OF SWR
C         TIMESTEPS.
C       - FIXED BUG IN RESIDUAL CALCULATIONS FOR CONSTANT STAGE REACHES ON
C         THE FIRST OUTER ITERATION AT THE START OF A STRESS PERIOD
C         ( Kkstp = 1 AND Kkiter = 1 ) WHEN SPECIFIED STAGE IS DIFFERENT
C         FROM THE PREVIOUS STRESS PERIOD
C
C     VERSION 0.26.0
C       - ADDED COMPREHENSIVE SCALING FOR DIFFUSIVE WAVE FLOW, REACH SEEPAGE TO
C         AQUIFER, AND EVAPORATION TO MAINTAIN STAGES AT DMINDPTH OR HIGHER.
C         SIGMOID SCALING OF BED ROUGHNESS, HEAD GRADIENT, AND EVAPORATION RATE
C         IS USED FROM DMINDPTH x 10 (F=1) TO DMINDPTH (F=0).
C
C     VERSION 0.25.0
C       - MOVED REACH DATA READ TO END OF TRANSIENT DATA TO ALLOW FOR USE OF
C         REACH BOTTOM TO EVALUATE IF THE STAGE PROVIDED SHOULD BE ADJUSTED.
C       - ADD CODE TO RESET INITIAL STAGE AND STAGE FOR SPECIFIC STRESS PERIODS
C         TO REACH BOTTOM + DMINDPTH IF SPECIFIED VALUE IS LESS THAN REACH BOTTOM
C       - IMPLEMENTED CONDUCTANCE SCALING FOR CASES WHERE THE INCREMENTAL VOLUME
C         IS LESS THAN THE CALCULATED QAQFLOW FLUX.  THIS MAY CAUSE PROBLEMS FOR
C         CONVERGENCE SO THIS APPROACH MAY NEED TO BE MODIFIED OR ELIMINATED AFTER
C         FURTHER TESTING AND EVALUATION.
C       - REVERTED PERTURBATION FOR FINITE DIFFERENCE JACOBIAN TO SIMPLE APPROACH
C         IDENTIFIED IN KELLY (2003).  SEEMS TO PROVIDE BETTER CONVERGENCE FOR
C         LARGE PROBLEMS
C       - REVISED ORDER OF VARIABLES IN INPUT ITEMS 1 AND 2.  INPUT ITEM 1 ONLY HAS PROBLEM
C         DIMENSION AND OUTPUT CONTROL VARIABLES.  INPUT ITEM 2 HAS SOLVER INDEPENDENT
C         SOLUTION VARIABLES (DLENCONV, TIMECONV, ISWT, ITWT, NTMIN, NTMAX, RTMULT, 
C         DMINGRAD, DMINDPTH, AND DMAXSTG).  INPUT ITEM 3 HAS SOLVER DEPENDENT
C         VARIABLES(ISOLVER, NOUTER, NINNER, IBT, TOLS, TOLR, TOLA, ISEWSOL, IPC, DSSOR,
C         BTFCT, BTRED, IBTPRT)
C       - ADDED ADAPTIVE TIMESTEP CONTROL FOR MAXIMUM STAGE CHANGE (DMAXSTG) PER
C         SWR TIMESTEP
C       - IMPLEMENT INACTIVE REACHES???
C       - ADDITIONAL STRUCTURE ROUTINES???
C
C     VERSION 0.24.0
C       - ADD UPSTREAM SPATIAL WEIGHTING OPTION THAT IS APPLIED TO
C         THE DIFFUSIVE WAVE EQUATION
C       - ADD REVISED EXPONENTIAL (SIGMOID) SCALING OF MANNING'S VALUES
C         BELOW DMNDEPTH
C       - ADDED DMNDEPTH AS USER DEFINED PARAMETER - PREVIOUSLY WAS HARDWIRED
C         AT 1.0D-03
C       - ADDED FORCING TERM (ETA) FOR KRYLOV SOLVER TO REDUCE TENDENCY TO
C         OVERSOLVE THE LINEAR SYSTEM - BASED ON JONES AND WOODWARD (2001)
C       - EVALUATE
C       - MINOR REVISIONS TO BICGSTAB SOLVER TO TERMINATE INNER ITERATIONS
C         WHEN THE L2NORM OF s IS LESS THAN TOLERANCE BASED ON FORCING 
C         TERM (ETA) - PREVIOUSLY THE SOLVER WAS TERMINATED IF THE L2NORM
C         OF s WAS LESS THAN TOLR - THIS WAS AN INCORRECT IMPLEMENTATION OF
C         PSEUDOCODE IN BARRETT ET AL. (1994)
C       - ELIMINATED PCG AS OPTIONAL KRYLOV ITERATIVE METHOD
C
C     VERSION 0.23.0
C       - REVISED JACOBIAN PERTURBATION BASED ON JONES AND WOODWARD (2001)
C
C     VERSION 0.22.0
C       - RESOLVED ISSUES WITH CASES WHERE NRCHGRP AND NSOLRG ARE LESS
C         THAN NREACHES - ISSUE WITH PS PASSED TO DIFFUSIVE WAVE AND
C         STRUCTURE FLOW FUNCTIONS
C       - ADDED ABILITY TO SPECIFY USE OF RELATIVE OR ABSOLUTE STOPPING
C         CRITERIA FOR RESIDUAL
C
C     VERSION 0.21.0
C       - REVISED ASSEMBLY OF JACOBIAN AND SOLUTION f AND X VECTORS TO 
C         REMOVE INACTIVE AND ACTIVE CELLS FROM SOLUTION MATRIX
C          o IMPLEMENTED TO REDUCE EXECUTION TIME FOR SIMULATIONS THAT
C            HAVE A SIGNIFICANT NUMBER OF CONSTANT STAGE OR INACTIVE 
C            REACHES.
C          o SOLUTION CONNECTIVINTY IS RECALCULATED EACH STRESS PERIOD
C            WHERE NEW ISWRBND CODES ARE READ FOR SELECT REACHES.
c
C     VERSION 0.20.0
C       - NEWTON-RAPHSON USING BANDED LU DECOMPOSITIONN ADDED AS OPTION
C         o IMPLEMENTED WHEN JAC%NBW * 2 .LT. NRCHGRP
C
C     VERSION 0.19.0
C       - ADDED BICONJUGATE GRADIENT STABILIZED SOLVER
C       - REVISED PCG SOLVER

C     VERSION 0.18.0
C       - ADDED STANDARD SUBROUTINE TO EVALUATE STOPPING CRITERIA THAT IS 
C         CONSISTENT ACROSS ALL SOLVERS - CURRENTLY USES L2NORMS AND 
C         A2NORMS (FROBENIUS) BUT COULD CONSIDER USE OF INFINITY VECTOR
C         AND MATRIX NORMS
C       - ADDED STANDARD SIMPLE RMS BACTRACKING SUBROUTINE THAT IS 
C         CONSISTENT ACROSS ALL SOLVERS
C       - ADDED RESTARTED GMRES SOLVER
C       - ADDED SSOR PRECONDITIONER
C       - FIXED BUG WITH UNINITIALIZED GROUP RESISTANCE VALUE FOR CURRENT
C         REACH GROUP IN UNCONTROLLED DIFFUSIVE WAVE FUNCTION
C
C     VERSION 0.17.0
C       - ADDITION OF GROUPING OF REACHES TO COARSEN DIFFUSIVE WAVE SOLUTION
C       - L2NORM AND A2NORM (FROBENIUS) USED AS STOPPING CRITERIA
C       - CREATED A MODIFIED VERSION OF STANDARD NEWTON SOLVER THAT
C         USED THE MODIFIED CRS JACOBIAN TO POPULATE THE FULL JACOBIAN
C       - INCLUDED A SIMPLIFIED RMS BASED BACKTRACKING IN THE MODIFIED
C         STANDARD NEWTON SOLVER
C       - FIXED BUG FOR CASES WHERE ENDING CONNECTIONS ARE STRUCTURE
C         TERMINATIONS INSTEAD OF CONSTANT STAGE REACHES
C
C     VERSION 0.16.0
C       - REFINE DIFFUSIVE WAVE CALCULATION BASED ON FIVE SAMPLE SOLUTIONS
C
C     VERSION 0.15.0
C       - ADDED DMINGRAD TO DEFINE MINIMUM GRADIENT FOR DIFFUSIVE
C         WAVE FLOW ROUTING
C       - REVISED SCALING OF RESISTANCE - AD HOC APPROACH
C         LINEAR FROM 0 TO M (gsm) FROM 1E-6 TO 1E-3.
C         ALSO REVISED THE CALCULATION OF bmin TO ALLOW FOR
C         FLOW FROM LARGE DEPTHS TO SMALL DEPTHS IF GRADIENT
C         PERMITS
C       - REMOVED SCALING OF WEIR WIDTH FOR LOW WATER DEPTH OVER
C         WEIR CREST IMPLEMENTED IN VERSION 0.14.0
C       - IMPLEMENT USE OF COMPRESSED STORAGE ARRAY FOR 
C         JACOBIAN IN ITERATIVE SOLVERS
C       - REVISED STORAGE OF THE JACOBIAN AND PRECONDITIONED FORM OF
C         JACOBIAN SO THAT IT IS ALLOCATED ONCE FOR EACH GRID INSTEAD
C         OF ALLOCATING AS AN AUTOMATIC ARRAY IN THE CALL TO THE SOLVERS.
C         HAD SOME PROBLEM WITH MEMORY ALLOCATION AND VARIABLE CRUSHING
C         WITH AUTOMATIC ARRAYS.  JACOBIAN (FJACC) AND THE PRECONDITIONED
C         FORM OF THE JACOBIAN (MJACC) ARE IN THE JAC DERIVED TYPE WITH
C         NNZ, IA, AND JA.
C       - ADDITION OF INCOMPLETE LU PRECONDITIONER
C       - ADDED JAC%IU VECTOR TO ALLOW EASY ACCESS TO START OF UPPER
C         PORTION OF JACOBIAN IN INCOMPLETE LU PRECONDITIONER ROUTINE
C       - REVISED JACOBI PRECONDITIONER SO THAT Mi (MJACC) ON THE 
C         DIAGONAL IS 0.0 IF THE JACOBIAN (FJACC) IS 0.0
C         THIS IS CONSISTENT WITH THE APPROACH USED IN THE
C         INCOMPLETE LU PRECONDITIONER
C
C     VERSION 0.14.0
C       - ADDED GZSHIFT TO ALLOW FOR VERTICAL ADJUSTMENT OF GENERIC
C         GEOMETRY DATA
C       - FIXED ERROR USING IGRID SPECIFIC DATA IN RP AND FM TO
C         DIMENSION LOCAL VARIABLES PRIOR TO SETTING POINTERS
C         FOR GRID
C       - IMPLEMENTED DIFFUSIVE WAVE FOR LOOPED 1-D NETWORKS WITH 
C         STRUCTURES
C       - INITIAL IMPLEMENTATION OF RAINFALL CONTROLED TIMESTEP
C         REDUCTION
C       - SCALING OF ROUGHNESS TO LIMIT UNCONTROLLED FLOW AT LOW
C         WATER DEPTH
C       - SCALING OF WEIR WIDTH TO REDUCE OSCILLATIONS FOR SMALL
C         WATER DEPTH ABOVE WEIR INVERT
C
C     VERSION 0.13.0
C       - ADDED DIFFUSIVE WAVE APPROXIMATION
C
C     VERSION 0.12.0
C       - RENAME VARIABLES TO BE CONSISTENT WITH DOCUMENTATION
C       - ATTACH STRUCTURES TO SPECIFIC REACHES INSTEAD OF USE OF 
C         GENERIC STRUCTURES
C       - REMOVED SEPERATE GROUP (IRG) DATA - INCORPORATED IN THE
C         RCHGRP DATA
C       - ADDED ISWRBND VARIABLE TO EACH REACH - ONLY THE CONSTANT
C         HEAD AND ACTIVE RCHGRP ARE IMPLEMENT - INACTIVE TO BE
C         IMPLEMENTED IN VERSION 0.13.0
C       - ADDED COMPRESSED ROW STORAGE APPROACH (IA AND JA VECTORS)
C         TO BE USED IN COMPRESSED FORM OF ITERATIVE SOLVERS
C         (NOTE: COMPRESSED STORAGE ITERATIVE SOLVERS NOT IMPLEMENTED)
C       - CALCULATING FULL REACH CONNECTIVITY FOR USE WITH REACHES
C         NOT USING THE STORAGE-INDICATION APPROACH (IROUTETYPE.GT.1)
C         
C     VERSION 0.11.0
C       - MULTI-LAYER GROUNDWATER CONNECTIONS FOR REACHES
C
C     VERSION 0.10.0
C       - ADDED QAQFLOW (TOLA) CONVERGENCE CRITERIA AND GWF2SWR7CV 
C         SUBROUTINE TO MODFLOW MAIN TO GIVE USER THE OPTION TO ACHIEVE
C         A SPECIFIED FLOW CONVERGENCE CRITERIA BETWEEN THE SWR FM AND
C         MODFLOW SOLUTIONS
C       - MOVED CALCULATION OF INFLOW IN SSWR_CALC_RGRESI TO AFTER CALCULATION OF
C         STRUCTURE OUTFLOWS BECAUSE OF DEPENDENCE ON INTERNAL US INFLOWS
C         ON BOUNDARY CALCULATIONS
C       - SINGLE PRECISION SWR WATER BUDGET ARRAYS FOR CONSISTENCY WITH
C         MODFLOW BUDGET TERMS WRITTEN TO THE LISTING FILE
C
C     VERSION 0.09.0
C       - ELIMINATED SEGMENTS
C       - MODIFIED GLOBAL SWR WATER BUDGET CALCULATION TO BE REACH-BASED
C         INSTEAD OF GROUP-BASED TO MAKE COMPARABLE TO MODFLOW SWR-SEEPAGE
C         TERMS.  ADDED FLOWDATA TYPE (SOLN INSTANCE) TO REACH STRUCTURE.
C       - IMPLEMENTED NEW LEAKAGE OPTIONS 
C         (0) SPECIFIED CONDUCTANCE
C         (2) USING HORIZONTAL HYDRAULIC CONDUCTIVITY
C         (3) HARMONIC MEAN OF LEAKANCE AND HORIZONTAL HYDRAULIC CONDUCTIVITY
C       - FIXED CENTRAL IN TIME SOLUTION FOR MULTIPLE SWR TIMESTEPS
C       - FIXED CALCULATED GBELEV FOR REACHES WITH IRREGULAR CROSS-SECTIONS (3),
C         SPECIFIED STAGE-VOLUME-WETTEDPERIMETER-SURFACE AREA RELATION (4), AND
C         REACHES THAT COVER THE ENTIRE CELL (5)
C       - REVISED ADAPTIVE TIMESTEP ALGORITHM SO NUMTIME(n) = NUMTIME(n-1) + RTMULT INSTEAD
C         OF NUMTIME(n) = NUMTIME(n-1) * RTMULT FOR INCREASED TIME RESOLUTION IN
C         SWR PACKAGE.  PREVIOUS APPROACH WAS VERY AGGRESSIVE FOR INCREASING AND
C         DECREASING THE SWR TIME RESOLUTION
C
      MODULE GWFSWRMODULE
        CHARACTER(LEN=64),PARAMETER :: VERSION_SWR =
     +'$Id: gwf2swr7.f 0.56.0 2012-05-18 15:00:00Z jdhughes $'
C
